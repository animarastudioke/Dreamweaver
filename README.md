<p align="center">
  <h1 align="center">Dreamweaver</h1>
  <p align="center">
    A game built with Claude Code Game Studios — a single Claude Code session
    run as a full game development studio.
    <br />
...
This repository already has the Claude Code Game Studios framework installed at its
root (`.claude/`, `CLAUDE.md`, and the supporting directories below). To start working:

1. **Clone this repository**:
   ```bash
   git clone https://github.com/animarastudioke/Dreamweaver.git
   cd Dreamweaver
   ```
...
Primary development and testing on **Windows 10** with Git Bash. All hooks use POSIX-compatible patterns (`grep -E`, not `grep -P`) and include fallbacks for missing tools, so they should run on macOS and Linux. The `notify.sh` hook uses PowerShell for Windows toast notifications and is a no-op elsewhere — desktop notifications on macOS/Linux are not yet wired. Cross-platform testing is ongoing.

## Credits

The Claude Code Game Studios framework (`.claude/`, `design/`, `docs/`, `production/`,
`src/` initial structure, and `CCGS Skill Testing Framework/`) was created by
[Donchitos](https://github.com/Donchitos) and is used here under its MIT license — see
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). Upstream project, issues, and
discussions: [github.com/Donchitos/Claude-Code-Game-Studios](https://github.com/Donchitos/Claude-Code-Game-Studios).

## License

This repository is licensed under the Apache License 2.0 — see [LICENSE](LICENSE).
The embedded Claude Code Game Studios framework retains its original MIT license;
see [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for the full text and attribution.
